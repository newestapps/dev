...
if (!function_exists('nwdev_mod')) {
    function nwdev_mod($dividendo, $divisor)
    {
...
if (!function_exists('nwdev_randomCPF')) {
    function nwdev_randomCPF($compontos)
    {
        ...
        $d1 = 11 - (nwdev_mod($d1, 11));
        if ($d1 >= 10) {
            $d1 = 0;
        }
        $d2 = $d1 * 2 + $n9 * 3 + $n8 * 4 + $n7 * 5 + $n6 * 6 + $n5 * 7 + $n4 * 8 + $n3 * 9 + $n2 * 10 + $n1 * 11;
        $d2 = 11 - (nwdev_mod($d2, 11));
        if ($d2 >= 10) {
            $d2 = 0;
        }
        $retorno = '';
...
if (!function_exists('nwdev_toXML')) {
    function nwdev_toXML($a = null, $format = 'array')
    {
        global $results;
        if ($format == 'json'):
